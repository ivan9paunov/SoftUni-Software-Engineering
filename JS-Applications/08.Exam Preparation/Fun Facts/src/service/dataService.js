...
const endpoints = {
    create: '/facts',
    allItems: '/facts?sortBy=_createdOn%20desc',
    myItem: (id) => `/facts/${id}`,
    // Adapt next ones to your BONUS task
    doAction: '/likes',
    alreadyInAction: (eventId, userId) => `/likes?where=factId%3D%22${eventId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
    counter: (eventId) => `/likes?where=factId%3D%22${eventId}%22&distinct=_ownerId&count`
};
